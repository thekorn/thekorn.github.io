...
const NodeUI: Component<{ node: Node }> = ({ node }) => {
  ...
  return (
    <div id={node.id} data-node class="absolute cursor-move ..." style={{
      left: `${node.x}px`, top: `${node.y}px`,
      width: `${node.width}px`, height: `${node.height}px`
    }}>
      <Dynamic component={shapes[node.shape]} class={isSelected() ? 'stroke-gray-500' : ''} />
      {node.title}
      <Port side="left" /><Port side="right" /><Port side="top" /><Port side="bottom" />
    </div>
  );
};
```
...
const EdgeUI: Component<{ id?: string; from: Accessor<Vec>; to: Accessor<Vec>; title?: Accessor<string> }> = ({ id, from, to, title }) => {
